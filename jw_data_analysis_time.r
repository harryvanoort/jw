...
MT1 <- gam(wf ~ s(julien, bs="cc")+ s(depth, by = zone, bs="cr") + s(year), data = dat, family=quasipoisson)


	# Residuals are still biased low
	# Some patterns in the residuals across julien
	# Try truncating last, indomplete year - appears to be affecting year effect
...
gam.check(MC)
	...
	qplot(dat$year, residuals.MC., data = df, geom = "point")
