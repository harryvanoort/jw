...
M11 <- cozigam(wf ~ s(julien, bs="cc")+ s(depth, by = zone, bs="cr"),family=poisson,data=dat)

	# did not converge

M12 <-  cozigam(wf ~ s(julien, bs="cc")+ s(depth, by = zone, bs="cr"),
	constraint="component",conv.crit.out=1e-3,family=poisson,data=dat)
